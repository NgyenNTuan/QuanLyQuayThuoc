...
import class_GUI.DonViQuyDoi_GUI;
...
public class Thuoc_EDIT_GUI extends JFrame {
	...
	private static JComboBox cboNgH,cboHC,cboNSX,cboNuocSX,cboDVT;
	...
	public Thuoc_EDIT_GUI(String maThuoc) {
		...
		lblHC.setFont(new Font("Arial", Font.PLAIN, 12));
		lblHC.setBounds(10, 133, 106, 24);
		panel_1.add(lblHC);
		...
		lblNSX.setBounds(10, 168, 106, 24);
		panel_1.add(lblNSX);
		...
		lblNuocSX.setBounds(10, 203, 106, 24);
		...
		cboNuocSX.setBounds(126, 204, 280, 24);
		...
		btnSave.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				if (kTraDinhDang()) {
					String maThuoc, tenThuoc, tenLoaiThuoc,hoatChat, tenNSX,quocGia,donViTinh, moTa;
					Float giaBan;
					int soLuong;
					...
					hoatChat = cboHC.getSelectedItem().toString().trim();
					moTa = txaMoTa.getText().trim();
					giaBan = Float.valueOf(txtGiaBan.getText().trim());
					soLuong = Integer.valueOf(txtSoLuong.getText().trim());
					Thuoc t = new Thuoc(maThuoc, tenThuoc, giaBan, soLuong, moTa, getNSX(tenNSX), getLT(tenLoaiThuoc), getDV(donViTinh), getQG(quocGia), getHC(hoatChat));
					try {
						if(!th_DAO.capnhat(t))
							throw new Exception();
						Thuoc_GUI.updateDL();
	...
	private static String getHC(String ten) {
		for(HoatChat s : lstHC) {
			if(s.getTenHC().trim().equalsIgnoreCase(ten.trim())) {
				return s.getMaHC().trim();
			}
		}
		return "";
	}
	private void docDLfromTable() {
		for(Thuoc t : Thuoc_GUI.lstT) {
			if(t.getMaThuoc().trim().equalsIgnoreCase(maThuoc1.trim())) {
				...
				cboHC.setSelectedItem(Thuoc_GUI.getHC(t.getMaHC().trim()));
